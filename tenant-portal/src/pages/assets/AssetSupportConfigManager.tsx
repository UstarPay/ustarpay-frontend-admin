...
export default function AssetSupportConfigManager({
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      console.error(`加载${scene}支持链配置失败`, error)
      message.error('加载支持链配置失败')
    } finally {
      setLoading(false)
    }
  }, [scene])
  ...
  const handleSave = async () => {
    try {
      ...
      message.success('支持链配置已保存')
      await loadData()
    } catch (error) {
      console.error(`保存${scene}支持链配置失败`, error)
      message.error('保存支持链配置失败')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert
        ...
        showIcon
        message="配置说明"
        description={`当前${scene === 'deposit' ? '充值' : '提现'}支持链数据源来自总后台币种列表。保存后，App 端和相关接口会按这里的启用结果进行过滤。`}
      />
